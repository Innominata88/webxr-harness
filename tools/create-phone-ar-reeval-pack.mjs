...
const manifestTag = "m2026-04-24-a";
...
const campaignDefs = [
  {
    key: "material_validity_manual",
    fileSuffix: "xr_ar_material_validity_manual_paired_1sets.json",
    suiteSuffix: "XR_AR_MATERIAL_VALIDITY_MANUAL",
    runIdBaseSuffix: "xr_ar_material_validity_manual",
    manifestRuns: 2,
    ...
    notes:
      "Manual validity smoke. Confirm real camera passthrough and stable pose before running longer AR material cohorts.",
  },
  {
    key: "material_probe_a_sanity",
    fileSuffix: "xr_ar_material_probe_a_paired_sanity_2sets.json",
    suiteSuffix: "XR_AR_MATERIAL_PROBE_A_SANITY",
    runIdBaseSuffix: "xr_ar_material_probe_a_sanity",
    manifestRuns: 4,
    ...
    surfaceMode: "basecolor",
    notes:
      "Short AR material sanity cohort. Use only if the manual validity smoke shows real passthrough and stable pose behavior.",
  },
  {
    key: "material_probe_a",
    fileSuffix: "xr_ar_material_probe_a_paired_5sets.json",
    suiteSuffix: "XR_AR_MATERIAL_PROBE_A",
    runIdBaseSuffix: "xr_ar_material_probe_a",
    manifestRuns: 10,
    ...
    surfaceMode: "basecolor",
    notes:
      "Full AR material reevaluation cohort. Run only after the manual validity smoke and short sanity cohort both pass.",
  },
];
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Phone AR Reevaluation Links</title>
    <style>
  ...
  <body>
    <h1>Phone AR Reevaluation Links</h1>
    <table>
      <thead>
        <tr>
          <th>Manifest</th>
...
async function main() {
  ...
  await fs.writeFile(path.join(outDir, "launcher-links-phone-ar-reeval.md"), linksMarkdown(links));
  await fs.writeFile(path.join(outDir, "launcher-links-phone-ar-reeval.csv"), linksCsv(links));
  await fs.writeFile(path.join(outDir, "launcher-links-phone-ar-reeval.html"), linksHtml(links));
  await fs.writeFile(
    ...
    `${JSON.stringify(
      {
        ...
        releaseTag,
        generatedAt: new Date().toISOString(),
        description: "Phone immersive-ar reevaluation pack: validity smoke plus AR material sanity/full cohorts for Samsung FE and Pixel 8a.",
      },
      null,
      2,
    )}\n`,
